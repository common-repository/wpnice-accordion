  ...
  class CSF_Field_dimensions extends CSF_Fields {
    ...
    public function render() {
      ...
      $args = wp_parse_args( $this->field, array(
        ...
        'width_placeholder'  => esc_html__( 'width', 'wpnice-accordion' ),
        'height_placeholder' => esc_html__( 'height', 'wpnice-accordion' ),
        'width'              => true,
        'height'             => true,
        'unit'               => true,
        'show_units'         => true,
      ...
      if ( ! empty( $args['width'] ) ) {
        ...
        echo ( ! empty( $args['width_icon'] ) ) ? '<span class="csf--label csf--icon">'. esc_attr($args['width_icon']) .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[width]' ) ) .'" value="'. esc_attr( $value['width'] ) .'"'. esc_attr($placeholder) .' class="csf-input-number'. esc_attr( $is_unit ) .'" step="any" />';
      ...
      if ( ! empty( $args['height'] ) ) {
        ...
        echo '<div class="csf--input">';
        echo ( ! empty( $args['height_icon'] ) ) ? '<span class="csf--label csf--icon">'. esc_attr($args['height_icon']) .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[height]' ) ) .'" value="'. esc_attr( $value['height'] ) .'"'. esc_attr($placeholder) .' class="csf-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( ! empty( $unit ) ) ? '<span class="csf--label csf--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
      }
