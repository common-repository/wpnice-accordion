<?php
	...
	function wpnice_accordion_add_color_picker( $hook ) {	 
	    if( is_admin() ) {       
			...
			wp_enqueue_style( 'wpnice-accordion-admin', plugins_url('../admin/assets/css/admin.css', __FILE__), array(), null, 'all' );	
			wp_enqueue_script( 'custom-script-handle', plugins_url( '../admin/assets/js/custom-script.js', __FILE__ ), array( 'wp-color-picker','jquery-ui-tabs' ), false, true ); 
	    }
	}